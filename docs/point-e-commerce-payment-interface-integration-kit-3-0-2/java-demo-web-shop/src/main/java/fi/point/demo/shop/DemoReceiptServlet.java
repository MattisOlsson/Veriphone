...
/**
 * Receipt handling servlet class.
 * @author Janis Stasans
 */
public class DemoReceiptServlet extends HttpServlet {
    ...
    protected final void doPost(final HttpServletRequest request, final HttpServletResponse response)
        ...
        out.println("<head>");
        out.println("<title>");
        out.println("Demo Receipt Page");
        out.println("</title>");
        out.println("</head>");
        out.println("<body>");
        out.println("<form action=\"order\" method=\"post\">");
        out.println("<h1>Demo Shop Receipt</h1>");
        out.println("<table>");
        ...
        System.out.println(String.format("DemoReceiptServlet Post from %s : %s ", request.getRemoteAddr(), request.getRemotePort()));        
    ...
    private TreeMap<String, String> gatherRequestParameters(final HttpServletRequest request) {
        ...
        parameters.put(parameterKey, request.getParameter(parameterKey));
        parameterKey = "s-t-1-36_order-note";
        parameters.put(parameterKey, request.getParameter(parameterKey));
        parameterKey = "t-f-14-19_order-timestamp";
        parameters.put(parameterKey, request.getParameter(parameterKey));
        parameterKey = "i-f-1-3_order-currency-code";
        parameters.put(parameterKey, request.getParameter(parameterKey));
